...
public class PhysicalWorld implements Serializable, JbSerializer.ObjectSigner, JbDeserializer.ObjectListener {
    ...
    */
	...
	/**
     * Add a complex polygonal object loaded from a JSON file made by Physics Body Editor to the PhysicalWorld
     * @param jsonName path to the JSON file
     * @param polygonScale a scale factor applied to all coordinates
     * @param type determine if the object is mobile (BodyType.DYNAMIC) or static (BodyType.STATIC)
     * @param position the coordinate of the PhysicalObject
     * @param orientation the orientation angle (in radians) of the PhysicalObject
     * @param sprite the associated Sprite
     ...
     * @throws InvalidSpriteNameException if the name was already used
     */
    public Body addComplexPolygonalObject(String jsonName, float polygonScale, BodyType type, Vec2 position, float orientation, Sprite sprite) throws InvalidSpriteNameException, InvalidPolygonException {

	   Vec2 origin = new Vec2(0,0);
        LinkedList<LinkedList<Vec2>> polygons = new LinkedList<LinkedList<Vec2>>();
        try {
            reader = new JsonReader(new FileReader(jsonName));
            /*Maintenu pour retrocompatibilité*/
            if (reader.peek().equals(JsonToken.BEGIN_OBJECT)) {
                reader.beginObject();
                while(reader.peek().equals(JsonToken.NAME)) { // For each category
                    String category = reader.nextName();
                    switch(category) {
                    case "rigidBodies":
                        reader.beginArray();
                        reader.beginObject();
                        while(reader.peek().equals(JsonToken.NAME)) { // For each token
                            String token = reader.nextName();
                            switch(token) {
                                //case "name" : System.out.println("Found : "+reader.nextString()); break;
                            case "origin" :
                             while(reader.peek().equals(JsonToken.BEGIN_OBJECT)) {
                              ...
                              while(reader.peek().equals(JsonToken.NAME)) {
                                  ...
                                  switch(value) {
                                  case "x":
                                      origin.x = (float)reader.nextDouble();
                                      break;
                                  case "y":
                                      origin.y = (float)reader.nextDouble();
                                      break;
                                  }
                              }
                             	reader.endObject();
                            ...
                            break;
                            case "polygons" :
                                reader.beginArray(); // Arrays of Arrays of Vec2

                                while(reader.peek().equals(JsonToken.BEGIN_ARRAY)) {
                                    reader.beginArray(); // Arrays of Vec2
                                    LinkedList<Vec2> polygon = new LinkedList<Vec2>();
                                    while(reader.peek().equals(JsonToken.BEGIN_OBJECT)) {
                                        reader.beginObject();
                                        float x=0, y=0;
                                        while(reader.peek().equals(JsonToken.NAME)) {
                                    ...
                                    reader.endArray(); // End Arrays of Vec2

                                }
                                reader.endArray(); // End Arrays of Arrays of Vec2
                                break;
                            default:
                                reader.skipValue();
                            }
        ...
        return addComplexPolygonalObject(polygons, origin, polygonScale, type, position, orientation, sprite);
    }
    
    public static Vec2 calculateCenterOfPolygon(LinkedList<Vec2> polygon) {
    ...
    return new Vec2((xMax - xMin)/2 + xMin, (yMax - yMin)/2 + yMin);
    
    }
    
    public static Vec2 calculateCenterOfPolygons(LinkedList<LinkedList<Vec2>> polygons) {
    
    float xMin = 0, xMax = 0, yMin = 0, yMax = 0;
    boolean first = true;
    for(LinkedList<Vec2> polygon:polygons) {
    	for(Vec2 point : polygon) {
    		if(first) {
    			xMin = xMax = point.x;
    			yMin = yMax = point.y;
	...
	/**
     * Add a complex polygonal object made from several polygons to the PhysicalWorld
     * @param polygons a list of polygons (made of list of Vec2)
     * @param origin coordinate of the center of the polygon (before polygonScale applied)
     * @param polygonScale a scale factor applied to all coordinates
     * @param type determine if the object is mobile (BodyType.DYNAMIC) or static (BodyType.STATIC)
     * @param position the coordinate of the PhysicalObject
     * @param orientation the orientation angle (in radians) of the PhysicalObject
     ...
     * @throws InvalidSpriteNameException if the name was already used
     */
    public Body addComplexPolygonalObject(LinkedList<LinkedList<Vec2>> polygons, Vec2 origin, float polygonScale, BodyType type, Vec2 position, float orientation, Sprite sprite) throws InvalidSpriteNameException, InvalidPolygonException {
        ...
        for(LinkedList<Vec2> polygon : polygons) {
            if(polygon.size() < 3) {
                throw new InvalidPolygonException("Not enough points for polygon "+polygon);
            }
            if(polygon.size() > Settings.maxPolygonVertices) {
                throw new InvalidPolygonException("Too much points (max : "+Settings.maxPolygonVertices+")  for polygon "+polygon);
            }
            PolygonShape shape = new PolygonShape();
            for(int i = 0 ; i < polygon.size() ; i++) {
            	polygon.set(i, new Vec2((polygon.get(i).x-origin.x) * 2 * polygonScale, (polygon.get(i).y-origin.y) * 2 * polygonScale));
            }

            
            shape.set(polygon.toArray(new Vec2[0]), polygon.size());
    ...
    /**
     * Add an shape-less object to the PhysicalWorld
     * @param type determine if the object is mobile (BodyType.DYNAMIC) or static (BodyType.STATIC)
     * @param position the coordinate of the PhysicalObject
     * @param orientation the orientation angle (in radians) of the PhysicalObject
     * @param sprite the associated Sprite
     ...
     * @throws InvalidSpriteNameException if the name was already used
     */
    public Body addShapelessPolygonalObject(BodyType type, Vec2 position, float orientation, Sprite sprite) throws InvalidSpriteNameException, InvalidPolygonException {
